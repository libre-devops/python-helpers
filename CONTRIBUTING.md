...
4. Run `just check`. It must pass: ruff lint, ruff format check and pytest.
5. Open a pull request using the template. CI then also scans every commit for secrets
   ...
   3.14 and on Windows and macOS, and builds the wheel. All of it must pass to merge.
...
2. Keep the layering, which `tests/test_package_surface.py` enforces:
   ...
   - Each vendor has a shared layer (`microsoft/*.py` and `microsoft/auth`) for what all of
     its features need: clouds, credentials, CLI runners, token checks, its config section.
...
9. Everything must be testable, and tests must not touch the network, a real `az` or a real
   ...
   fakes in `tests/fakes.py`: a fake `requests` adapter for HTTP, a fake subprocess runner
   for the Azure CLI, and a fake clock for anything that polls. Run `just test-311` as well
   as `just check`, so nothing newer than Python 3.11 slips in.
10. Never hard-code the tool's names. The command, environment variable prefix, config
    ...
    rename everything; `tests/test_rebrand.py` rebrands a copy and runs its suite to prove it.
