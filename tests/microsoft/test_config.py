...
def test_token_cache_defaults_to_a_file_and_is_read_for_delegated_profiles():
    device = config(auth="device-code", client_id=CLIENT_ID)
    assert parse(device).profiles["prod"].token_cache == "file"
    kept = config(auth="device-code", client_id=CLIENT_ID, token_cache="memory")
    assert parse(kept).profiles["prod"].token_cache == "memory"


@pytest.mark.parametrize(
    ("profile", "message"),
    [
        ({"token_cache": "disk"}, "token_cache must be one of memory, keychain, file"),
        ({"token_cache": "memory"}, "token_cache applies to"),
        (
            {"auth": "client-secret", "client_id": CLIENT_ID, "token_cache": "keychain"},
            "token_cache applies to",
        ),
    ],
)
def test_token_cache_is_checked(profile, message):
    with pytest.raises(ConfigError, match=message):
